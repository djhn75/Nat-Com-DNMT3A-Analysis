...
DO.Mean.SEM.Graphs <- function(SeuratObject, Features, ListTest=NULL, returnValues=FALSE, ctrl.condition=NULL, group.by = "condition", returnPlot=FALSE){ 
  ...
  for (condition in df.melt.sum$condition) {
    df.melt.orig.sub <- df.melt.orig[df.melt.orig$condition %in% condition,]
    df.melt.sum[df.melt.sum$condition %in% condition,]$SEM <- SEM(df.melt.orig.sub$Mean)
  }
  
  stat.df <- data.frame(Mean = SeuratObject[["RNA"]]@data[Features,],
             condition = SeuratObject[[group.by]][,group.by])
  ...
  p<-ggplot(df.melt.sum, aes(x = condition, y = Mean, fill = condition))+
    ...
    stat_compare_means(data=stat.df, comparisons = ListTest, method = "wilcox", size=3)+
...
#perform SEM Graphs, similar to DO.mean.SEM.graphs but takes the mean not over cells, it takes the mean over the mean per sample
#' @author Mariano Ruz Jurado
#' @param SeuratObject # combined object
#' @param Features # vector containing featurenames
#' @param ListTest # List for which conditions t-test will be performed, if NULL always against provided CTRL 
...
#' @param ctrl.condition # set your ctrl condition, relevant if running with empty comparison List
#' @param group.by # select the seurat object slot where your conditions can be found, default conditon
DO.Mean.SEM.Graphs.modified <- function(SeuratObject, Features, ListTest=NULL, returnValues=FALSE, ctrl.condition=NULL, group.by = "condition", returnPlot=FALSE){ 
  require(ggplot2)
  require(ggpubr)
  #require(tidyverse)
  require(reshape2)
  ...
  #add Mean calculated over sample means
  df.melt.sum$Mean <- NA
  for (condition in df.melt.sum$condition) {
    df.melt.orig.sub <- df.melt.orig[df.melt.orig$condition %in% condition,]
    df.melt.sum[df.melt.sum$condition %in% condition,]$Mean <- mean(df.melt.orig.sub$Mean)
  }
